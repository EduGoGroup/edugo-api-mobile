...
// RequireDockerRunning verifica que Docker esté corriendo
// Salta el test si Docker no está disponible
func RequireDockerRunning(t *testing.T) {
	t.Helper()
	// Implementación básica - se puede mejorar con exec.Command("docker", "ps")
	// Por ahora solo verifica la variable de entorno
	if os.Getenv("DOCKER_HOST") == "" && os.Getenv("CI") == "" {
		// En local, asumir que Docker Desktop está corriendo
		// En CI, testcontainers maneja Docker automáticamente
	}
}
