...
setup_postgres() {
    ...
    if [ -n "$table_count" ] && [ "$table_count" -gt 0 ]; then
        ...
        print_warning "PostgreSQL schema needs to be created"
        echo -e "\n  ${YELLOW}IMPORTANTE:${NC} Las migraciones locales han sido eliminadas."
        echo -e "  Las migraciones ahora vienen de ${CYAN}edugo-infrastructure${NC}\n"
        echo -e "  ${YELLOW}Para crear el schema:${NC}"
        echo -e "    1. Las migraciones se ejecutarán automáticamente al iniciar la aplicación"
        echo -e "    2. O ejecuta manualmente: ${CYAN}make migrate-up${NC}\n"
        echo -e "  ${CYAN}Ubicación de migraciones:${NC}"
        echo -e "    edugo-infrastructure/postgres/migrations/*.up.sql\n"

        print_info "Esquema se creará al iniciar la aplicación (vía edugo-infrastructure)"
    fi

    # Note: Seed data is now handled via testcontainers in tests
    # or separate development scripts (not in migrations)
    print_info "Seed data debe manejarse vía testcontainers o scripts de desarrollo"
    echo -e "  ${CYAN}Para desarrollo local:${NC}"
    echo -e "    - Tests usan testcontainers con datos de prueba"
    echo -e "    - Crear script separado si necesitas datos de desarrollo\n"
